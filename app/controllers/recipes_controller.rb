class RecipesController < ApplicationController
  ...
  def show
    ...
    @youtube_video = find_youtube_video(@recipe.name)
  end
  ...
  def find_youtube_video(recipe_name)
    search_response = YOUTUBE_API_SERVICE.list_searches('id,snippet', q: "#{recipe_name} рецепт", type: 'video')
    first_video = search_response.items.first
    first_video&.id&.video_id
  end
